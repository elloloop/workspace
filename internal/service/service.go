...
// the auth middleware: the user every decision is made against and the
// project (isolation shard) the request operates in.
...
// tests; nil falls back to time.Now and a random hex id.
...
func (s *Service) allowed(ctx context.Context, p Principal, workspaceID string, rel Role) (bool, error) {
	return s.engine.Check(ctx, p.ProjectID, "workspace", workspaceID, string(rel), p.UserID)
}
...
func (s *Service) requireWorkspace(ctx context.Context, p Principal, workspaceID string, rel Role) (*Workspace, error) {
	w, err := s.repo.GetWorkspace(ctx, p.ProjectID, workspaceID)
	if err != nil {
		return nil, err
	}
